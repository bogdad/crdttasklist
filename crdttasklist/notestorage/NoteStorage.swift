...
class NoteStorage {
  ...
  func mergeNotes(_ remoteNotes: Notes) -> (MergeStatus, Notes) {
    ...
    localDict.merge(
      remoteDict,
      uniquingKeysWith: { (left, right) -> Note in
    ...
    print(
      "mergeNotes: local count \(localNotes.count) remote count \(remoteNotes.count) new local count \(newLocalNotes.count)"
    )
    print(
      "mergeNotes: numMissingLocally \(numMissingLocally) numMissingRemotely \(numMissingRemotely)")
    ...
    return (
      MergeStatus(needsUpload: needsUpload, needsLocalRedraw: needsLocalRedraw), newLocalNotes
    )
  }
