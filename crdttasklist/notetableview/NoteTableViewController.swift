...
class NoteTableViewController: UITableViewController {
  ...
  override func viewDidLoad() {
    ...
    if !NoteStorage.shared.isStorageLinked() {
      let linkToSorageViewController = self.storyboard?.instantiateViewController(
        withIdentifier: "LinkToStorageViewController")
      ...
      NotificationCenter.default.addObserver(
        self, selector: #selector(notesChangedRemotely),
        name: NSNotification.Name("notesChangedRemotely"), object: nil)
      remoteTimer = Timer.scheduledTimer(
        timeInterval: 30, target: self, selector: #selector(runTimedCode), userInfo: nil,
        repeats: true)
    }
  ...
  override func tableView(
    ...
    var note = noteByIndex(indexPath.row)
    let setChecklistAction = UIContextualAction(style: .normal, title: "Checklist") {
